...
<tr>
<td colspan="2">💻 <b>Your own clones</b><br>Pick a folder and Shepherd reads the repository from its <code>origin</code>, links the checkout and watches every pull request in it — one step. Then <i>Start an agent…</i> hands a task you type to Claude Code on a fresh <code>agent/…</code> branch off the default branch, with the same caps — as many tasks side by side in one repository as you like, each in its own worktree. Shepherd itself pushes nothing.</td>
</tr>
<tr>

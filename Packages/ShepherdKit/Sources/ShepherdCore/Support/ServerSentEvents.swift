...
/// Reads the text out of the Anthropic streaming shape.
///
/// Pure and provider-shaped rather than provider-owned, for the same reason
/// ``ServerSentEventParser`` is: the app target can only test this against a live key, and the
/// interesting cases — a `content_block_delta` for a second block, a `message_delta` with no
/// text in it, an `error` frame arriving mid-answer — are exactly the ones a live key would never
/// reproduce on demand.
///
/// Only `text_delta` deltas are read. A stream that carries anything else (a tool-use input
/// delta, say) contributes nothing to the draft rather than contributing its JSON.
public enum AnthropicStreamDecoder {
    /// The text this frame adds to the answer.
    /// - Parameter event: A dispatched frame.
    /// - Returns: The delta's text, or `nil` when the frame carries none.
    public static func textDelta(in event: ServerSentEvent) -> String? {
        ...
        // The `event:` line is authoritative when it is there, but it is not required to be: the
        // frame's own `type` says the same thing, and trusting the payload means a stream sent
        // without event names still decodes.
        guard let frame = try? JSONDecoder().decode(Frame.self, from: Data(event.data.utf8)),
              frame.type == "content_block_delta" || event.event == "content_block_delta"
        else { return nil }
        guard frame.delta?.type == nil || frame.delta?.type == "text_delta" else { return nil }
        guard let text = frame.delta?.text, !text.isEmpty else { return nil }
        return text
    }

    /// The message an `error` frame carries, when the stream failed after the headers.
    ///
    /// A streamed request can fail with a 200 and then say so in the body, which is the one
    /// failure mode a status-code check cannot catch.
    /// - Parameter event: A dispatched frame.
    ...
    public static func errorMessage(in event: ServerSentEvent) -> String? {
        ...
        guard let frame = try? JSONDecoder().decode(Frame.self, from: Data(event.data.utf8)),
              frame.type == "error" || event.event == "error"
        else { return nil }
        let message = frame.error?.message?.trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    public static func text(in body: String) -> String {
        ServerSentEventParser.events(in: body).compactMap(textDelta(in:)).joined()
    }
}

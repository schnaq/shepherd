...
| --- | --- | --- |
| `ShepherdCore` | Foundation only | Domain models (`PullRequestSummary`, `ReviewDraft`, `ChangedFile`, …), `AgentDetector` + the bundled `agent-registry.json`, `FilePrioritizer`, `InboxGrouper`, `PullRequestDigestBuilder`, `BulkTriagePlan`, `AutoDelegationPolicy` + its rules/ledger values, `SavedReply` and `ReviewTemplate` (insertion, per-repo matching, the empty-draft prefill rule), `ClaimExtractor` + `EvidenceChecker` + `ClaimsEvidenceReport` (claims vs. evidence, ADR 0026), the `ConditionalCache` and `Sleeping` protocols |

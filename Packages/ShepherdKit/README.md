...
| --- | --- | --- |
| `ShepherdCore` | Foundation only | Domain models (`PullRequestSummary`, `ReviewDraft`, `ChangedFile`, …), `AgentDetector` + the bundled `agent-registry.json`, `FilePrioritizer`, `InboxGrouper`, `PullRequestDigestBuilder`, `BulkTriagePlan`, `AutoDelegationPolicy` + its rules/ledger values, the `ConditionalCache` and `Sleeping` protocols |
